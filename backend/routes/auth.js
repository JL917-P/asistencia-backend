...
router.post('/register-begin', async (req, res) => {
  try {
    ...
    if (u.rows.length) {
      ...
      userId = makeUUID();
      await pool.query('INSERT INTO users (id, username, display_name) VALUES ($1,$2,$3)', [userId, username, displayName || username]);
    }
    ...
    const options = generateRegistrationOptions({
      ...
      excludeCredentials: creds.rows.map(r => ({ id: Buffer.from(r.credential_id, 'base64url'), type: 'public-key' }))
    });
...
router.post('/auth-begin', async (req, res) => {
  try {
    ...
    const allowCredentials = rows.rows.map(r => ({ id: Buffer.from(r.credential_id, 'base64url'), type: 'public-key' }));

    const options = generateAuthenticationOptions({
      timeout: 60000, rpID: RP_ID, allowCredentials, userVerification: 'preferred',
    });
    authChallenges.set(userId, options.challenge);
    res.json({ options, userId, displayName: u.rows[0].display_name });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
router.post('/auth-complete', async (req, res) => {
  try {
    ...
    if (!cred.rows.length) return res.status(404).json({ error: 'credential not found' });
    ...
    await pool.query('UPDATE credentials SET sign_count=$1 WHERE id=$2', [newCounter, id]);
